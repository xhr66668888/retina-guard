#!/usr/bin/env bash
# Build the Apple Silicon (arm64) Retina Guard macOS package.
...
DMG="RetinaGuard-mac-arm64.dmg"
ARCHIVE="RetinaGuard-mac-arm64.tar.gz"
VOL="Retina Guard arm64"
ARCH="arm64"
MIN_MACOS="11.0"
...
/usr/libexec/PlistBuddy -c "Add :LSArchitecturePriority:0 string arm64" "$APP/Contents/Info.plist"
/usr/libexec/PlistBuddy -c "Set :LSMinimumSystemVersion $MIN_MACOS" "$APP/Contents/Info.plist"

echo "Building Retina Guard for macOS Apple Silicon ($ARCH) ..."
clang \
    -arch "$ARCH" \
...
if ! file "$APP_BIN" | grep -q "arm64"; then
    echo "Error: $APP_BIN is not an arm64 Mach-O binary." >&2
    file "$APP_BIN" >&2
    exit 1
fi
